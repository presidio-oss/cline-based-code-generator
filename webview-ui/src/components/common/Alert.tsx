			...
			<div style={{ display: "flex", width: "100%" }} onClick={toggleExpand}>
				{isAccordion && (
					<div
						...
						style={{ marginRight: "6px" }}></div>
				)}
				{isAccordion && (
					<div className={`codicon ${alertIcons[type]}`} style={{ marginRight: "6px", fontWeight: "bold" }}></div>
				)}
				{header && <div style={{ flexGrow: 1, fontWeight: "bold" }}>{header}</div>}
			</div>
			<div style={{ paddingTop: "4px" }}>{isExpanded && children}</div>
		</div>
	)
}

...
const CustomInstructionsMenu = ({ isExpanded, onToggleExpand }: CustomInstructionsMenuProps) => {
	...
	const toggleTextInstruction = () => {
		...
		setAllInstructionsEnabled(
			(fileInstructions?.every((i) => i.enabled === true) ?? false) && customInstructions !== undefined && value,
		)
	}
		...
		<div
			...
			<div
				...
				<CollapsibleSection isHovered={isHoveringCollapsibleSection} style={{ cursor: "pointer" }}>
					<span style={{ color: "var(--vscode-foreground)", whiteSpace: "nowrap" }}>Custom Instructions:</span>
					<span
						style={{
							whiteSpace: "nowrap",
							overflow: "hidden",
						...
						(customInstructions && isCustomInstructionsEnabled)
							? `${fileInstructions
									...
									isCustomInstructionsEnabled
										? ", "
										: ""
								}${customInstructions && isCustomInstructionsEnabled ? "Default" : ""}`
							: "None"}
					</span>
					<span
			...
			{isExpanded && (
				<div style={{ padding: "8px 0" }}>
					<div
						...
						Custom instructions allow you to define specific behaviors for the AI assistant. Enable instructions to
						include them in every conversation with the AI.
					</div>
					<div className="space-y-2">
						{fileInstructions?.map((instruction, index) => (
							<div key={instruction.name} style={{ margin: "6px 0" }}>
								<VSCodeCheckbox checked={instruction.enabled} onClick={() => toggleInstruction(index)}>
									{instruction.name}
								</VSCodeCheckbox>
							</div>
						))}
						{customInstructions && (
							<>
								<div key={customInstructions} style={{ margin: "6px 0" }}>
									<VSCodeCheckbox checked={isCustomInstructionsEnabled} onClick={toggleTextInstruction}>
								...
								<p
									...
									These default instructions are textual instructions provided in the settings, which are added
									to the end of the system prompt sent with every request.
								</p>
							</>
						)}
						{!customInstructions && (!fileInstructions || fileInstructions.length < 1) && (
							<p
								...
								To leverage capabilities, configure instructions via the settings page "Custom Instructions" field
								or by uploading .md files [ex: place coding-style.md in the .hai/instructions folder].
							</p>
						)}
					</div>
				</div>

...
export const ExtensionStateContextProvider: React.FC<{
	...
	useEffect(() => {
		...
		// Set up experts button clicked subscription
		expertsButtonClickedSubscriptionRef.current = UiServiceClient.subscribeToExpertsButtonClicked(EmptyRequest.create(), {
			onResponse: () => {
				// When experts button is clicked, navigate to experts view
				console.log("[DEBUG] Received experts button clicked event from gRPC stream")
				navigateToExperts()
			},
			onError: (error) => {
				console.error("Error in experts button clicked subscription:", error)
			},
			onComplete: () => {
				console.log("Experts button clicked subscription completed")
			},
		})
		...
		return () => {
			...
			if (expertsButtonClickedSubscriptionRef.current) {

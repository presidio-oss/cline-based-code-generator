...
description: "Remote browser support allows HAI to utilize a remote Chrome instance, leveraging authentication tokens and session cookies relevant to certain web development test cases."
icon: globe-pointer
---

The Remote Browser feature in HAI allows the AI assistant to interact with web content directly through a controlled browser instance. This enables several powerful capabilities:
...
## Remote Browser in HAI

### What is Remote Browser?

Remote Browser allows HAI to view and interact with websites directly. This feature enables HAI to:
...
You can ask HAI to use the browser with simple instructions:
...
HAI can only use one browser at a time. If you want to visit a different website, you can either:

- Ask HAI to navigate to a new URL within the same browser session
- Ask HAI to close the current browser and open a new one

#### Browser Must Be Closed Before Using Other Tools

If you want HAI to edit files or run commands after using the browser, you must first ask it to close the browser:
...
The browser has a fixed viewport size (900x600 pixels by default), similar to a small laptop screen. HAI will share screenshots after each action so you can see exactly what it sees.

#### Console Logs

HAI captures browser console logs, which can be helpful for debugging web applications. These logs are included with each screenshot.

### Common Use Cases

- **Web Development**: Test your websites and web applications
- **UI/UX Review**: Get feedback on website design and usability
- **Content Research**: Have HAI browse websites to gather information
...
- **If the browser seems stuck**: Ask HAI to close the browser and try again
...
#### Configure HAI in VS Code:

1. Open VS Code settings
2. Search for "HAI: Chrome Executable Path"
3. Set the value to the path of your Chrome executable (e.g., `C:\Program Files\Google\Chrome\Application\chrome.exe`)

HAI should now be able to use the Remote Browser feature from within WSL.

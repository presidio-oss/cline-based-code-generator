---
title: "Anthropic"
description: "Learn how to configure and use Anthropic Claude models with HAI. Covers API key setup, model selection, and advanced features like prompt caching."
---
...
3.  **Create a Key:** Click "Create Key". Give your key a descriptive name (e.g., "HAI").
...
1.  **Open HAI Settings:** Click the settings icon (⚙️) in the HAI panel.
...
When extended thinking is in HAI, the model generates `thinking` content blocks that detail its internal reasoning. These insights are then incorporated into its final response.
HAI users can leverage this by checking the `Enable Extended Thinking` box below the model selection menu after selecting a Claude Model from any provider.

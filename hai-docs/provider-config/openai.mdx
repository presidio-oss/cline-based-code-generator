---
title: "OpenAI"
description: "Learn how to configure and use official OpenAI models with HAI."
---

HAI supports accessing models directly through the official OpenAI API.
...
3.  **Create a Key:** Click on "Create new secret key". It's good practice to give your key a descriptive name (e.g., "HAI API Key").
...
HAI is compatible with a variety of OpenAI models, including but not limited to:
...
### Configuration in HAI

1.  **Open HAI Settings:** Click the settings gear icon (⚙️) in the HAI panel.
...
- **Azure OpenAI Service:** If you are looking to use the Azure OpenAI service, please note that specific documentation for Azure OpenAI with HAI may be found separately, or you might need to configure it as an OpenAI-compatible endpoint if such functionality is supported by HAI for custom configurations.

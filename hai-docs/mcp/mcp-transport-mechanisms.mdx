...
description: "Learn about the two primary transport mechanisms for communication between HAI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases."
---

Model Context Protocol (MCP) supports two primary transport mechanisms for communication between HAI and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (HAI) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as HAI
...
- Single-client scenarios (one HAI instance per server)
...
1. The client (HAI) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from your HAI instance
...
STDIO servers run locally on the same machine as HAI, which has several important implications:
...
- **Execution**: Starts and stops with HAI (child process lifecycle)
...
- Start when needed by HAI
- Not require network configuration
- Need to be installed alongside HAI or via a package manager
...
## Configuring Transports in HAI

For detailed information on configuring STDIO and SSE transports in HAI, including examples, see [Configuring MCP Servers](/mcp/configuring-mcp-servers).

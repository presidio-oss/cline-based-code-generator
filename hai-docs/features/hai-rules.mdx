...
| ---------------- | ----------------------- | ---------------------------------------------- |
| **Windows**      | `Documents\HAI\Rules`   | Uses system Documents folder                   |
| **macOS**        | `~/Documents/HAI/Rules` | Uses user Documents folder                     |
| **Linux/WSL**    | `~/Documents/HAI/Rules` | May fall back to `~/HAI/Rules` on some systems |

> **Note for Linux/WSL users**: If you don't find your global rules in `~/Documents/HAI/Rules`, check `~/HAI/Rules` as the location may vary depending on your system configuration and whether the Documents directory exists.

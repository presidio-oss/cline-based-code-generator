...
When your conversation approaches the model's context window limit, HAI automatically summarizes it to free up space and keep working. 
...
HAI monitors token usage during your conversation. When you're getting close to the limit, he:
...
Previously, HAI would truncate older messages when hitting context limits. This meant losing important context from earlier in the conversation.
...
1. HAI uses a [summarization prompt](https://github.com/cline/cline/blob/main/src/core/prompts/contextManagement.ts) to request a summary of the conversation.

2. Once the summary is generated, HAI replaces the conversation history with a [continuation prompt](https://github.com/cline/cline/blob/main/src/core/prompts/contextManagement.ts#L69) that asks HAI to keep working and provides the summary as context.
...
<Note>
When using other models, HAI automatically falls back to the standard rule-based context truncation method, even if Auto Compact is enabled in settings.
</Note>

...
HAI's terminal integration lets you bring your terminal output directly into your conversations with HAI. Instead of copying and pasting error messages or command results, you can send them to HAI with a simple right-click in the terminal.
...
When you're working in the VSCode terminal and see output you want to discuss with HAI:

1. Right-click in the terminal
2. Select "Add to HAI" from the context menu
3. The terminal output is immediately sent to the HAI chat panel
...
Instead of switching context to the HAI chat panel and typing a description of the problem, you can send the terminal output directly to HAI with just a couple of clicks.

Alternatively, you can use the [`@terminal`](/features/at-mentions/terminal-mentions) mention to send the full terminal output to HAI.
...
When you use the right-click terminal integration, HAI:
...
This gives HAI the full context it needs to provide accurate help with terminal-related issues.

## Behind the Scenes

The terminal integration uses a clever technique to capture terminal output:

1. When you trigger the integration, HAI:
...
- **Use terminal integration for error messages**: When you encounter an error in the terminal, sending it to HAI often results in faster resolution than trying to describe the error.
...
- **Combine terminal outputs with file mentions**: After sending terminal output to HAI, you can enhance your question by mentioning relevant files using the @ mentions feature.
...
Next time you're staring at a cryptic error message in your terminal, try using HAI's terminal integration instead of copying and pasting. You'll get more accurate help because HAI can see the complete terminal context with proper formatting.
...
<Tip>
	**Quick Fix**: Most terminal issues can be resolved by switching to bash in the HAI settings and increasing the shell

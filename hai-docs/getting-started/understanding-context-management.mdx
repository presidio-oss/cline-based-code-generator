...
description: "Context is key to getting the most out of HAI"
---

> **Quick Reference**
>
> - Context = The information HAI knows about your project
> - Context Window = How much information HAI can hold at once
...
<Frame caption="In a world of infinite context, the context window is what HAI currently has available">
	<img
		src="https://storage.googleapis.com/cline_public_images/docs/assets/image%20(2).png"
		alt="In a world of infinite context, the context window is what HAI currently has available"
	/>
</Frame>

Think of working with HAI like collaborating with a thorough, proactive teammate:

### How Context is Built

HAI actively builds context in two ways:

1. **Automatic Context Gathering (i.e. HAI-driven)**
...
2. **User-Guided Context**
    ...
    - Answer HAI's questions
    - Guide focus areas
    - Share design thoughts and requirements

**Key Point**: HAI isn't passive - it actively seeks to understand your project. You can either let it explore or guide its focus, especially in [Plan Mode](/features/plan-and-act).

### Context & Context Windows

Think of context like a whiteboard you and HAI share:

-   **Context** is all the information available:
    -   What HAI has discovered
    -   What you've shared
...
-   **Context Window** is the size of the whiteboard itself:
    ...
    -   When the whiteboard is full, HAI automatically summarizes the conversation to free up space
...
HAI provides a visual way to monitor your context window usage through a progress bar:
...
- When HAI seems to lose context

**Tip**: With [Auto Compact](/features/auto-compact), HAI can now handle long conversations automatically. When combined with [Focus Chain](/features/focus-chain), you can work on complex projects that span multiple context windows without losing progress.

## Automatic Context Management

HAI includes intelligent features to manage context automatically:

### Default Settings You Should Keep On

**Focus Chain** - Enabled by default in v3.25. HAI generates a todo list at task start and keeps it in context so the thread doesn't drift. You can edit the markdown to add or reorder steps and HAI will adapt. [Learn more about Focus Chain](/features/focus-chain).

**Auto Compact** - Always on. As the context window reaches its limit, HAI creates a comprehensive summary, replaces the bloated history, and continues where it left off. Decisions, code changes, and state are preserved. [Learn more about Auto Compact](/features/auto-compact).
...
For substantial features, refactors, or integrations. HAI investigates your codebase, asks targeted questions, then writes `implementation_plan.md`. It creates a fresh task with distilled, high-value context. [Learn more about Deep Planning](/features/slash-commands/deep-planning).
...
### Memory Bank + .hairules
For non-trivial projects. The Memory Bank captures project knowledge as Markdown in your repo. `.hairules` are version-controlled instructions that align HAI's behavior with your team. [Learn more about Memory Bank](/prompting/hai-memory-bank) and [HAI Rules](/features/hai-rules).
...
3. **Knowledge Transfer Docs**
    - Switch to plan mode and ask HAI to document everything you've accomplished so far, along with the remaining steps, in a markdown file.
...
3. **Team Projects**
    - Share common context files (consider using [.hairules](/features/hai-rules) files in project roots)
...
- You can @ links and have the webpage's context added to HAI (docs, blogs, etc.)
...
HAI already does a lot of context work for you - [Focus Chain](/features/focus-chain), [Auto Compact](/features/auto-compact), and the planning flow are designed to keep the thread intact across long horizons. The goal is to help HAI maintain consistent understanding of your project across sessions.

...
export async function activate(context: vscode.ExtensionContext) {
	...
	const openClineInNewTab = async () => {
		...
		const panel = vscode.window.createWebviewPanel(VscodeWebviewProvider.TAB_PANEL_ID, "HAI Build", targetCol, {
			enableScripts: true,
	...
	*/
	...
	// Wait for CacheService to be initialized before accessing it
	await sidebarWebview.controller.waitForCacheServiceInitialization()
	const apiConfiguration = sidebarWebview.controller.cacheService.getApiConfiguration()
	context.subscriptions.push(
		...new InlineEditingProvider()
			.withCurrentMode(currentMode)
			.withContext(context)
			.withController(sidebarWebview.controller)
	...
	context.subscriptions.push(
		vscode.commands.registerCommand("hai.fixWithCline", async (range: vscode.Range, diagnostics: vscode.Diagnostic[]) => {
			const context = await getContextForCommand(range, diagnostics)
			if (!context) {
				return
			}
...
function setupHostProvider(context: ExtensionContext) {
	...
	const getCallbackUri = async () => `${vscode.env.uriScheme || "vscode"}://presidio-inc.hai-build-code-generator`

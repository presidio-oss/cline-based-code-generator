...
export async function validateWorkspacePath(workspacePath: string): Promise<void> {
	...
	try {
		const testFile = path.join(workspacePath, ".hai_write_test")
...
export async function initializeGitRepository(workspacePath: string): Promise<boolean> {
	...
	try {
		...
		await execa("git", ["config", "user.name", "Hai Evaluation"], { cwd: workspacePath })
		await execa("git", ["config", "user.email", "hai@example.com"], { cwd: workspacePath })

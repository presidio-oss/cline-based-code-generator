...
import { name as extensionName, version as extensionVersion } from "../../../../package.json"
...
export class TelemetryService {
	...
	private static readonly EVENTS = {
		...
		TASK: {
			...
			// Tracks when individual HAI rules are toggled on/off
	...
	public constructor(private provider: PostHogClientProvider) {
		...
		this.initLangfuseClient().then((langfuse) => {
	...
	public async initLangfuseClient() {
		const config = await HaiConfig.getLangfuseConfig()
	...
	public async updateTelemetryState(didUserOptIn: boolean): Promise<void> {
		...
		if (vscode.env.isTelemetryEnabled) {
			...
			// Only show warning if user has opted in to HAI telemetry but VS Code telemetry is disabled
			if (didUserOptIn) {
				...
				if (isVsCodeHost) {
					void HostProvider.window
						.showMessage({
							...
							message:
								"Anonymous HAI error and usage reporting is enabled, but VSCode telemetry is disabled. To enable error and usage reporting for this extension, enable VSCode telemetry in settings.",
							options: {
								items: ["Open Settings"],
							},
						})
					...
					void HostProvider.window.showMessage({
						type: ShowMessageType.WARNING,
						message: "Anonymous HAI error and usage reporting is enabled, but host telemetry is disabled.",
					})
				}
			}
	...
	public captureTaskCreated(ulid: string, apiProvider?: string) {
		...
		this.createLangfuseTraceClient(ulid, true)
	}
	...
	public captureTaskRestarted(ulid: string, apiProvider?: string) {
		...
		// Start a new trace in Langfuse
		this.createLangfuseTraceClient(ulid)
	}
	...
	/**
	 * Records when individual HAI rules are toggled on/off

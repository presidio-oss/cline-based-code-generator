...
export class ConversationTelemetryService {
	...
	public isOptedInToConversationTelemetry(): boolean {
		...
		const isConversationTelemetryEnabled =
			vscode.workspace.getConfiguration("hai").get<boolean>("conversationTelemetry") ?? false
	...
	private async initializeTracer() {
		try {
			...
			const resource = new Resource({
				[ATTR_SERVICE_NAME]: "hai-extension",
			...
			// Get a tracer
			this.tracer = trace.getTracer("hai-conversation-tracer")

...
export class VectorizeCodeAgent extends EventEmitter {
    ...
    constructor(srcFolder: string, embeddingConfig: EmbeddingConfiguration, buildContextOptions: HaiBuildContextOptions, contextDir = HaiBuildDefaults.defaultContextDirectory) {
        super();
    ...
    private async job(filePaths?: string[]) {
        ...
        const defaultExcludeDirs: string[] = [...HaiBuildDefaults.defaultDirsToIgnore, faissWithoutContextDir, faissWithContextDir];
        ...
        const codeFiles = (filePaths && filePaths.length > 0) ? new Set(filePaths) : new Set(getCodeFiles(srcFolder, excludedFolders, [HaiBuildDefaults.defaultRepoHashFileName]));
        ...
        for (const codeFilePath of codeFiles) {
            ...
            const fileContentHashMD5 = createHash('md5').update(JSON.stringify({ fileContent, embeddingConfig: this.embeddingConfig, buildContextOptions: this.buildContextOptions })).digest('hex')
        ...
        for (const codeFilePath of codeFilesSet) {
            ...
            if (!fileContentHashMD5) {
                ...
                fileContentHashMD5 = createHash('md5').update(JSON.stringify({ fileContent, embeddingConfig: this.embeddingConfig, buildContextOptions: this.buildContextOptions })).digest('hex')
            }
            ...
            const docs: Document[] = texts.map((text) => ({ pageContent: text, id, metadata: { source: codeFilePath, fileName, fileContentHashMD5 } }));
            try {

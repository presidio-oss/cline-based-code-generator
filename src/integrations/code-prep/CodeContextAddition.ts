...
export class CodeContextAdditionAgent extends EventEmitter {
    ...
    private async processFile(codeFilePath: string) {
        ...
        const fileContentHashMD5 = createHash('md5').update(JSON.stringify({ fileContent, filePath: codeFilePath })).digest('hex')
    ...
    private async processStream(iterator: AsyncGenerator<ApiStreamChunk, any, any>, destinationFilePath: string, fileContentHashMD5: string, codeFilePath: string): Promise<void> {
        // Stop processing stream when user aborts
        if (this.abortController.signal.aborted || !this.running) {
            return;
        }
    ...
    private async processFilesConcurrently(codeFiles: Set<string>) {
        ...
        for (const codeFilePath of codeFiles) {
            ...
            const taskPromise = this.processFile(codeFilePath)
                .finally(() => {

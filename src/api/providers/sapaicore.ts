...
export class SapAiCoreHandler implements ApiHandler {
	...
	async validateAPIKey(): Promise<boolean> {
		try {
			if (!this.options.sapAiCoreClientId || !this.options.sapAiCoreClientSecret || !this.options.sapAiCoreTokenUrl) {
				return false
			}
			...
			const url = `${this.options.sapAiCoreBaseUrl}/v2/lm/deployments?$top=1&$skip=0`

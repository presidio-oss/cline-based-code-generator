...
export async function onDidShowAnnouncement(controller: Controller, _request: EmptyRequest): Promise<Boolean> {
	try {
		...
		await customUpdateState(controller.context, "lastShownAnnouncementId", controller.latestAnnouncementId)

		// Get the updated lastShownAnnouncementId value after the update
		const lastShownAnnouncementId = await customGetState(controller.context, "lastShownAnnouncementId")

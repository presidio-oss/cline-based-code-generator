...
export class LmStudioHandler implements ApiHandler {
	...
	async *createMessage(systemPrompt: string, messages: Anthropic.Messages.MessageParam[]): ApiStream {
		...
		try {
			...
			throw new Error(
				"Please check the LM Studio developer logs to debug what went wrong. You may need to load the model with a larger context length to work with HAI prompts. Alternatively, try enabling Compact Prompt in your settings when working with a limited context window.",
			)
		}
	}
	...
	async validateAPIKey(): Promise<boolean> {
		try {
			...
			console.error("Error validating LM Studio credentials: ", error)
			return false
		}
	}
}

...
export class ToolExecutor {
	...
	public async executeTool(block: ToolUse): Promise<void> {
		...
		switch (block.name) {
			...
			case "replace_in_file": {
				...
				try {
					...
					if (diff) {
						...
						// open the editor if not done already.  This is to fix diff error when model provides correct search-replace text but HAI throws error
					...
					if (block.partial) {
						...
						// Mark the file as edited by HAI to prevent false "recently modified" warnings
			...
			case "report_bug": {
				...
				try {
					if (block.partial) {
						...
						const clineVersion =
							vscode.extensions.getExtension("presidio-inc.hai-build-code-generator")?.packageJSON.version ||
							"Unknown"
						...
						if (text || (images && images.length > 0) || (reportBugFiles && reportBugFiles.length > 0)) {
							...
							try {
								...
								params.set("hai-version", clineVersion)
			...
			default:
				await this.customToolUseHandlers(block)
				break
		}
	}
	...
	async customToolUseHandlers(block: ToolUse) {
		switch (block.name) {
			case "find_relevant_files": {
				const sharedMessageProps: ClineSayTool = {
					tool: "findRelevantFiles",
					path: getReadablePath(this.cwd),
				}
					...
					if (block.partial) {
						...
						if (this.buildContextOptions && this.task) {
							...
							await this.say(
								"tool",
								completeMessage(
									`Found ${relevantFiles.length} files\n\n` + buildTreeString(relevantFiles, this.cwd),
								),
								undefined,
								undefined,
								false,
							)
							this.pushToolResult(relevantFiles.toString(), block)
						} else {
							await this.say("tool", completeMessage("No relevant files found"), undefined, undefined, false)
							this.pushToolResult("No files found", block)
						}
						break
					}
				} catch (error) {
					await this.handleError("finding relevant files", error, block)
					break
				}
			}
			case "code_security_scan": {
				...
				try {
					if (block.partial) {
						...
						if (aiEditedFiles.length > 0) {
							const didApprove = await this.askApproval("tool", block, partialMessage(""))
							if (!didApprove) {
								this.pushToolResult("The user denied this operation.", block)
								break
							}
							const codeSecurityScan = new CodeScanner().withApiConfig(this.apiConfiguration).build()
							const results = await codeSecurityScan.scanFiles(aiEditedFiles, this.cwd)
							const resultString = results
								.map((result) => `Security Scan Result for ${result.path}:\n\n${result.result}`)
								.join("\n")
							await this.say("tool", completeMessage(resultString), undefined, undefined, false)
							this.pushToolResult(resultString.toString(), block)
						} else {
							const message = "No security issues found"
							await this.say("tool", completeMessage(message), undefined, undefined, false)
							this.pushToolResult(message, block)
						}
					}
					break
				} catch (error) {
					await this.handleError("code security scan", error, block)
					break
				}
			}
			case "custom_expert_context": {
				...
				try {
					if (block.partial) {
						...
						if (!query) {
							this.taskState.consecutiveMistakeCount++
							this.pushToolResult(await this.sayAndCreateMissingParamError("custom_expert_context", "query"), block)
							await this.saveCheckpoint()
							break
						}
						if (!expertName) {
							this.taskState.consecutiveMistakeCount++
							this.pushToolResult(
								await this.sayAndCreateMissingParamError("custom_expert_context", "expertName"),
								block,
							)
							await this.saveCheckpoint()
							break
						...
						const searchResults = await expertManager.search(query, expertName, this.cwd)
						...
						this.pushToolResult(searchResults, block)
						await this.saveCheckpoint()
						break
					}
				} catch (error) {
					await this.handleError("searching expert context", error, block)
					await this.saveCheckpoint()
					break
				}
			}

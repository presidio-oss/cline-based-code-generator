...
export class Task {
	...
	async ask(
		...
		// If this HAI instance was aborted by the provider, then the only thing keeping us alive is a promise still running in the background, in which case we don't want to send its result to the webview as it is attached to a new instance of HAI now. So we can safely ignore the result of any active promises, and this class will be deallocated. (Although we set HAI = undefined in provider, that simply removes the reference to this instance, but the instance is still alive until this promise resolves or rejects.)
	...
	private async startTask(task?: string, images?: string[], files?: string[]): Promise<void> {
		...
		// if the extension process were killed, then on restart the clineMessages might not be empty, so we need to set it to [] when we create a new HAI client (otherwise webview would show stale messages from previous session)
	...
	private async initiateTaskLoop(userContent: UserContent): Promise<void> {
		...
		while (!this.taskState.abort) {
			...
			// There is a MAX_REQUESTS_PER_TASK limit to prevent infinite requests, but HAI is prompted to finish the task as efficiently as he can.
			...
			if (didEndLoop) {
				...
				// this.say(
				// 	"tool",
				// 	"HAI responded with only text blocks but has not called attempt_completion yet. Forcing him to continue with task..."
				// )
				nextUserContent = [
					{
						type: "text",
		...
		if (this.taskState.consecutiveMistakeCount >= 3) {
			...
			const { response, text, images, files } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "HAI uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 4 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
		...
		try {
			...
			telemetryService.captureTokenUsage(
				...
				this.api.getModel().id,
				{
					...
					apiProvider:
						this.mode === "plan"
							? this.apiConfiguration.planModeApiProvider
							: this.apiConfiguration.actModeApiProvider,

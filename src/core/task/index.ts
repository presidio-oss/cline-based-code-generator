...
export class Task {
	...
	async *attemptApiRequest(previousApiReqIndex: number): ApiStream {
		...
		const disableBrowserTool = vscode.workspace.getConfiguration("hai").get<boolean>("disableBrowserTool") ?? false
		...
		const preferredLanguage = getLanguageKey(
			vscode.workspace.getConfiguration("hai").get<LanguageDisplay>("preferredLanguage"),
		)
		...
		switch (block.type) {
			...
			case "tool_use":
				...
				switch (block.name) {
							...
							if (block.partial) {
								...
								if (this.shouldAutoApproveToolWithPath(block.name, relPath)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`HAI wants to ${fileExists ? "edit" : "create"} ${path.basename(relPath)}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "tool")
						...
						try {
							if (block.partial) {
								...
								if (
									...
									showNotificationForApprovalIfAutoApprovalEnabled(`HAI wants to execute a command: ${command}`)
							...
							if (block.partial) {
								...
								if (mcp_arguments) {
									try {
										...
										await this.say(
											"error",
											`HAI tried to use ${tool_name} with an invalid JSON argument. Retrying...`,
										)
										pushToolResult(
											formatResponse.toolError(
								...
								if (this.shouldAutoApproveTool(block.name) && isToolAutoApproved) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`HAI wants to use ${tool_name} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
						...
						try {
							if (block.partial) {
								...
								if (this.shouldAutoApproveTool(block.name)) {
									...
									showNotificationForApprovalIfAutoApprovalEnabled(
										`HAI wants to access ${uri} on ${server_name}`,
									)
									this.removeLastPartialMessageIfExistsWithType("say", "use_mcp_server")
									const didApprove = await askApproval("use_mcp_server", completeMessage)
									if (!didApprove) {
					...
					case "report_bug": {
						...
						try {
							if (block.partial) {
								...
								const haiVersion =
									vscode.extensions.getExtension("presidio-inc.hai-build-code-generator")?.packageJSON
										.version || "Unknown"
								const systemInfo = `VSCode: ${vscode.version}, Node.js: ${process.version}, Architecture: ${os.arch()}`
								const providerAndModel = `${(await customGetState(this.getContext(), "apiProvider")) as string} / ${this.api.getModel().id}`
								...
								if (text || images?.length) {
									...
									try {
										...
										params.set("hai-version", haiVersion)
					...
					default:
						await this.customToolUseHandlers(block, pushToolResult, askApproval, handleError)
						break
				}
				break
		}
		...
		if (this.consecutiveMistakeCount >= 3) {
			...
			const { response, text, images } = await this.ask(
				...
				this.api.getModel().id.includes("claude")
					...
					: "HAI uses complex prompts and iterative task execution that may be challenging for less capable models. For best results, it's recommended to use Claude 3.7 Sonnet for its advanced agentic coding capabilities.",
			)
			if (response === "messageResponse") {
				userContent.push(
					...[
		...
		if (
			...
			if (this.autoApprovalSettings.enableNotifications) {
				showSystemNotification({
					...
					message: `HAI has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests.`,
				})
			}
			await this.ask(
				"auto_approval_max_req_reached",
				`HAI has auto-approved ${this.autoApprovalSettings.maxRequests.toString()} API requests. Would you like to reset the count and proceed with the task?`,
			)
	...
	customToolUseDescriptionHandlers(block: ToolUse) {
		switch (block.name) {
	...
	async customToolUseHandlers(
		...
		switch (block.name) {
			case "find_relevant_files": {
				const sharedMessageProps: ClineSayTool = {
					tool: "findRelevantFiles",
					path: getReadablePath(cwd),
				}
				const completeMessage = (content: string) =>
					JSON.stringify({
						...sharedMessageProps,
						content: content,
					} satisfies ClineSayTool)
				const partialMessage = (content: string) =>
					JSON.stringify({
						...sharedMessageProps,
						content: content,
					} satisfies ClineSayTool)
				try {
					if (block.partial) {
						...
						if (this.buildContextOptions && this.task) {
							...
							await this.say(
								"tool",
								completeMessage(`Found ${relevantFiles.length} files\n\n` + buildTreeString(relevantFiles, cwd)),
								undefined,
								false,
							)
							pushToolResult(relevantFiles.toString())
						} else {
							await this.say("tool", completeMessage("No relevant files found"), undefined, false)
							pushToolResult("No files found")
						}
						break
					}
				} catch (error) {
					await handleError("finding relevant files", error)
					break
				}
			}
			case "code_security_scan": {
				const sharedMessageProps: ClineSayTool = {
					tool: "codeSecurityScan",
					path: getReadablePath(cwd),
				}
				const completeMessage = (content: string) =>
					JSON.stringify({
				...
				try {
					if (block.partial) {
						...
						const aiEditedFiles = Array.from(this.filesEditedByAI)
						if (aiEditedFiles.length > 0) {
							const didApprove = await askApproval("tool", partialMessage(""))
							if (!didApprove) {
								pushToolResult("The user denied this operation.")
								break
							}
							const codeSecurityScan = new CodeScanner().withApiConfig(this.apiConfiguration).build()
							const results = await codeSecurityScan.scanFiles(aiEditedFiles, cwd)
							const resultString = results
								.map((result) => `Security Scan Result for ${result.path}:\n\n${result.result}`)
								.join("\n")
							await this.say("tool", completeMessage(resultString), undefined, false)
							pushToolResult(resultString.toString())
						} else {
							const message = "No security issues found"
							await this.say("tool", completeMessage(message), undefined, false)

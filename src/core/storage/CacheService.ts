...
export class CacheService {
	...
	setWorkspaceStateBatch(updates: Partial<LocalState>): void {
		...
		// Update cache immediately for all keys
		Object.entries(updates).forEach(([key, value]) => {
			;(this.workspaceStateCache as any)[key] = value
	...
	/**
	 * Convenience method for getting embedding configuration
	 */
	getEmbeddingConfiguration(): EmbeddingConfiguration {
		if (!this.isInitialized) {
			throw new Error(CACHE_SERVICE_NOT_INITIALIZED)
		}

		// Construct embedding configuration from cached component keys
		return this.constructEmbeddingConfigurationFromCache()
	}

	/**
	 * Convenience method for setting embedding configuration
	 */
	setEmbeddingConfiguration(embeddingConfiguration: EmbeddingConfiguration): void {
		if (!this.isInitialized) {
			throw new Error(CACHE_SERVICE_NOT_INITIALIZED)
		}
		...
		this.setGlobalStateBatch({
			...
			embeddingAwsRegion: awsRegion,
			embeddingOpenAiBaseUrl: openAiBaseUrl,
			embeddingOpenAiModelId: openAiModelId,
			embeddingAzureOpenAIApiInstanceName: azureOpenAIApiInstanceName,
			embeddingAzureOpenAIApiEmbeddingsDeploymentName: azureOpenAIApiEmbeddingsDeploymentName,
			embeddingAzureOpenAIApiVersion: azureOpenAIApiVersion,
			embeddingOllamaBaseUrl: ollamaBaseUrl,
		...
		this.setSecretsBatch({
			...
			embeddingOpenRouterApiKey: openRouterApiKey,
			embeddingAwsAccessKey: awsAccessKey,
			embeddingAwsSecretKey: awsSecretKey,
			embeddingAwsSessionToken: awsSessionToken,
			embeddingOpenAiApiKey: openAiApiKey,
			embeddingOpenAiNativeApiKey: openAiNativeApiKey,
			embeddingAzureOpenAIApiKey: azureOpenAIApiKey,
		})
	}

	/**
	 * TAG:HAI - Convenience methods for HAI-specific configuration
	 */
	getHaiConfiguration() {
		if (!this.isInitialized) {
			throw new Error(CACHE_SERVICE_NOT_INITIALIZED)
		...
		return {
			enableInlineEdit: this.globalStateCache["enableInlineEdit"] ?? true,
			buildContextOptions: this.workspaceStateCache["buildContextOptions"]
				? {
						...this.workspaceStateCache["buildContextOptions"],
						systemPromptVersion: this.workspaceStateCache["buildContextOptions"].systemPromptVersion ?? "v3",
					}
				: {
			...
			buildIndexProgress: this.workspaceStateCache["buildIndexProgress"],
			expertPrompt: this.globalStateCache["expertPrompt"],
			expertName: this.globalStateCache["expertName"],
			isDeepCrawlEnabled: this.globalStateCache["isDeepCrawlEnabled"],
		}
	}
	...
	private populateCache(state: any): void {
		...
		const { enableInlineEdit, buildContextOptions, buildIndexProgress, expertPrompt, expertName, isDeepCrawlEnabled } =
			state.haiConfiguration
		...
		const globalStateFields = {
			...
			// TAG:HAI - HAI-specific global state fields
			enableInlineEdit: enableInlineEdit ?? true,
			expertPrompt,
			expertName,
			isDeepCrawlEnabled,

			// Embedding configuration global state fields
			embeddingProvider,
			embeddingModelId,
			embeddingAwsRegion,
		...
		const secretsFields = {
			...
			// Embedding configuration secret fields
			embeddingApiKey,
	 ...
	 * Construct embedding configuration from cached component keys
	 */
	private constructEmbeddingConfigurationFromCache(): EmbeddingConfiguration {
		return {
			provider: (this.globalStateCache["embeddingProvider"] as EmbeddingProvider) || "none",
			modelId: this.globalStateCache["embeddingModelId"],
			apiKey: this.secretsCache["embeddingApiKey"],
			openRouterApiKey: this.secretsCache["embeddingOpenRouterApiKey"],
			awsAccessKey: this.secretsCache["embeddingAwsAccessKey"],
			awsSecretKey: this.secretsCache["embeddingAwsSecretKey"],
			awsSessionToken: this.secretsCache["embeddingAwsSessionToken"],
			awsRegion: this.globalStateCache["embeddingAwsRegion"],
			openAiBaseUrl: this.globalStateCache["embeddingOpenAiBaseUrl"],
			openAiApiKey: this.secretsCache["embeddingOpenAiApiKey"],
			openAiModelId: this.globalStateCache["embeddingOpenAiModelId"],
			openAiNativeApiKey: this.secretsCache["embeddingOpenAiNativeApiKey"],
			azureOpenAIApiKey: this.secretsCache["embeddingAzureOpenAIApiKey"],
			azureOpenAIApiInstanceName: this.globalStateCache["embeddingAzureOpenAIApiInstanceName"],
			azureOpenAIApiEmbeddingsDeploymentName: this.globalStateCache["embeddingAzureOpenAIApiEmbeddingsDeploymentName"],
			azureOpenAIApiVersion: this.globalStateCache["embeddingAzureOpenAIApiVersion"],
			ollamaBaseUrl: this.globalStateCache["embeddingOllamaBaseUrl"],
			ollamaModelId: this.globalStateCache["embeddingOllamaModelId"],
			maxRetries: this.globalStateCache["embeddingMaxRetries"],
		}
	}
}

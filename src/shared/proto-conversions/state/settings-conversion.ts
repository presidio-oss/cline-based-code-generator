...
function convertEmbeddingProviderToProto(provider: EmbeddingProvider | undefined): ProtoEmbeddingProvider {
	if (!provider) {
		return ProtoEmbeddingProvider.NONE
	}
	...
	switch (provider) {
		case "none":
			return ProtoEmbeddingProvider.NONE
		case "bedrock":
			return ProtoEmbeddingProvider.BEDROCK_EMBEDDING
		case "openai-native":
			return ProtoEmbeddingProvider.OPENAI_NATIVE_EMBEDDING
		case "openai":
			return ProtoEmbeddingProvider.OPENAI_EMBEDDING
		case "ollama":
			return ProtoEmbeddingProvider.OLLAMA_EMBEDDING
		default:
			return ProtoEmbeddingProvider.NONE
	}
}
...
function convertProtoToEmbeddingProvider(protoProvider: ProtoEmbeddingProvider): EmbeddingProvider {
	switch (protoProvider) {
		case ProtoEmbeddingProvider.NONE:
			return "none"
		case ProtoEmbeddingProvider.BEDROCK_EMBEDDING:
			return "bedrock"
		case ProtoEmbeddingProvider.OPENAI_NATIVE_EMBEDDING:
			return "openai-native"
		case ProtoEmbeddingProvider.OPENAI_EMBEDDING:
			return "openai"
		case ProtoEmbeddingProvider.OLLAMA_EMBEDDING:
			return "ollama"
		default:
			return "none"
	}
...
/**
 * Converts domain EmbeddingConfiguration objects to proto EmbeddingConfiguration objects
 */
export function convertEmbeddingConfigurationToProtoEmbeddingConfiguration(
	config: EmbeddingConfiguration,
): ProtoEmbeddingConfiguration {
	return ProtoEmbeddingConfiguration.create({
...
/**
 * Converts proto EmbeddingConfiguration objects to domain EmbeddingConfiguration objects
 */
export function convertProtoEmbeddingConfigurationToEmbeddingConfiguration(
	protoConfig: ProtoEmbeddingConfiguration,
): EmbeddingConfiguration {
	return {
		// Provider configuration
		provider: protoConfig.provider !== undefined ? convertProtoToEmbeddingProvider(protoConfig.provider) : undefined,

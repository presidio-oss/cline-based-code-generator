...
HAI's Git integration brings AI assistance directly to your version control workflow. Generate commit messages without leaving your editor.
...
<Frame>
	<img
		src="https://storage.googleapis.com/hai_public_images/docs/assets/generate-commit-message-with-hai.png"
		alt="Generate Commit Message with HAI"
	/>
</Frame>

1. Make your changes and stage them in Git
2. Click the robot icon in the Source Control view or run the "Generate Commit Message with HAI" command
3. HAI analyzes your changes and generates a descriptive commit message
...
When you use HAI's commit message generation feature, here's what happens behind the scenes:

1. HAI retrieves the current Git diff using `getWorkingState()`
...
This process uses your current HAI API configuration, so the quality of the generated messages matches your chosen AI model.
...
Next time you're struggling to write a good commit message, try using HAI's commit message generation. You'll save time and improve your version control workflow with AI assistance right where you need it.

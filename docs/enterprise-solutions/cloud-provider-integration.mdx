...
HAI supports major cloud providers like AWS Bedrock and Google's Cloud Vertex; whichever your team currently uses is appropriate, and there's no need to change providers to utilize HAI's features.
...
3. Note the IP address of your VPC endpoint, open HAI's settings menu, and select `AWS Bedrock`from the API Provider dropdown.
